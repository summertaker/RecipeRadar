...
local options = {
	...
	name = L["Recipe Radar"] .. " " .. RECIPERADAR_VERSION,
	args = {
		RealmAvailability = {
			type = "toggle",
   ...
   if (RecipeRadar.db.profile.CurrentContinent == 0) then
      ...
      regions = { GetMapZones(RecipeRadar.db.profile.CurrentContinent) }
...
function RecipeRadar_NewRegion(region)
   ...
   RecipeRadar.db.profile.CurrentRegion = region
...
function RecipeRadar_FormatVendorNote(note, include_parens)
   ...
   if (include_parens) then
      return format(TEXT(PARENS_TEMPLATE), format(L["%s Vendor"], note))
   end
